...
def bollinger_bands(df, n):
    ...
    b1 = 4 * MSD / MA
    B1 = pd.Series(b1, name='Lower Band')
    df = df.join(B1)
    b2 = (df['Close'] - MA + 2 * MSD) / (4 * MSD)
    B2 = pd.Series(b2, name='Upper Band')
    df = df.join(B2)
    return df
...
def ICHIMOKU(ohlc):
    ...
    tenkan_sen = pd.Series((ohlc["High"].rolling(window=9).mean() + ohlc["Low"].rolling(window=9).mean()) / 2, name="Ichimoku Conversion Line") ## conversion line
    kijun_sen = pd.Series((ohlc["High"].rolling(window=26).mean() + ohlc["Low"].rolling(window=26).mean()) / 2, name="Ichimoku Base Line") ## base line

    senkou_span_a = pd.Series(((tenkan_sen + kijun_sen) / 2), name="Ichimoku Leading Span A") ## Leading span
    senkou_span_b = pd.Series(((ohlc["High"].rolling(window=52).mean() + ohlc["Low"].rolling(window=52).mean()) / 2), name="Ichimoku Leading Span B")
    chikou_span = pd.Series(ohlc["Close"].shift(-26).rolling(window=26).mean(), name="Ichimoku Lagging Span")

    return pd.concat([ohlc, tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b, chikou_span], axis=1)
